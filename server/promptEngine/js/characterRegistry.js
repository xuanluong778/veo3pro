 ...
 */
export function sanitizeCharacterId(raw) {
    const s = String(raw ?? '')
        .trim()
        .toLowerCase();
    if (!s || !ID_RE.test(s))
...
export const CHARACTER_REGISTRY = {
    'host-neutral-vn': {
        ...
        visualLock: 'Adult Vietnamese presenter, neutral warm expression, short neat black hair, medium skin tone, plain cream crew-neck top, natural soft key on face, no glasses, minimal jewelry (optional simple watch only), consistent facial proportions and age read.',
    ...
    'mascot-lung-cute': {
        ...
        visualLock: 'Soft rounded lung-shaped mascot, pastel pink-coral gradient body, large friendly dot eyes, small blush marks, stubby limbs, felt-stop-motion texture, fixed proportions and silhouette, no photoreal human features.',
    ...
    'product-hand-only': {
        ...
        visualLock: 'Single adult hand (medium skin tone, short clean nails) holding the product; wrist and forearm edge only; no face, no second person, consistent hand identity across scenes.',
...
export function listRegistryPublicMeta() {
 ...
 */
export function parseRequestedCharacterIds(raw) {
    ...
    for (const item of list) {
        ...
        if (!id) {
            if (invalidSamples.length < 8)
                invalidSamples.push(trimmed.slice(0, 64));
            continue;
        }
        if (seen.has(id))
            continue;
    ...
    if (invalidSamples.length)
        return { ok: false, code: 'INVALID_FORMAT', samples: invalidSamples };
    if (unknown.length)
        return { ok: false, code: 'UNKNOWN_IDS', unknownIds: unknown };
    return { ok: true, ids: normalized.slice(0, MAX_CHARACTER_IDS) };
}
export function resolveCharacterProfiles(ids) {
    const unknown = [];
    const out = [];
    const seen = new Set();
    ...
    if (unknown.length)
        throw new UnknownCharacterIdError(unknown);
    return out;
}
/**
 * Deterministic CHARACTER line payload from locked profiles (no model paraphrase).
 */
export function formatCharacterCentralFromProfiles(profiles, mode) {
    const modeLine = `consistency mode: ${mode} | VISUAL_LOCK_REGISTRY_ONLY — do not invent, alter, age, or restyle these identities; match locked profiles exactly in every scene; no AI-driven character variation.`.replace(/\s+/g, ' ').trim();
    if (!profiles.length)
        return `Central subject not specified | ${modeLine}`;
    const blocks = profiles.map((p) => {
        const parts = [`[${p.id}] ${p.displayName}`, `VISUAL_LOCK: ${p.visualLock}`];
        if (p.wardrobeLock)
            parts.push(`WARDROBE_LOCK: ${p.wardrobeLock}`);
        if (p.negativeHints)
            parts.push(`CHAR_NEG_HINTS: ${p.negativeHints}`);
        return parts.join(' | ');
    });
...
export function focusLabelsFromProfiles(profiles, quantity) {
    if (!profiles.length)
        return [];
...
//# sourceMappingURL=characterRegistry.js.map