...
export const DEFAULT_PROMPT_DNA: PromptDNA = {
  globalStyle:
    'Single cohesive cinematic world; one film LUT and contrast curve for the entire run; no genre or decade hop.',
  cameraBase:
    '35–50mm equivalent lens family; stable horizon; motivated blocking; no unmotivated handheld shake.',
  lightingBase:
    'Soft key with controlled rim; consistent color temperature and shadow density across every scene.',
  motionBase:
    'Motivated camera moves only; match on action; no random whip-pan or speed ramp unless SCENE_FUNCTION demands one clear beat.',
  environmentBase:
    'Continuity geography and production design; same materials palette; no random location redesign between scenes.',
  negativePrompt:
    'AI must not invent new lens pack, new LUT, new aspect, new film stock, or new lighting scheme; no text watermark; no burned-in subtitles',
  audioBase:
    'Diegetic-first mix; score supports picture; spoken dialogue only in the compiled AUDIO line (verbatim segment); no mic peaking.',
};

export function mergePromptDNA(partial?: Partial<PromptDNA>): PromptDNA {
  return { ...DEFAULT_PROMPT_DNA, ...partial };
}

/** Builds the DNA line block prepended to every compiled scene prompt (system-owned). */
export function buildDnaLineStrings(dna: PromptDNA, meta: DnaRuntimeMeta): string[] {
...
export function buildNegativeFromDNA(dna: PromptDNA, userNegative: string, fullSuffix: string): string {
